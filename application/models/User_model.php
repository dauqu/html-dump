...
class User_model extends CI_Model {
    ...
    public function add_instructor() {
        ...
        if ($validity == false) {
            ...
            $data['student_first_name'] = html_escape($this->input->post('first_name'));
            $data['student_last_name'] = html_escape($this->input->post('last_name'));
    ...
    public function edit_user($user_id = "") { // Admin does this editing
        ...
        if ($validity) {
            ...
            $data['father_contact_no'] = html_escape($this->input->post('father_contact_no'));
            $data['father_email'] = html_escape($this->input->post('father_email'));
            $data['mother_name'] = html_escape($this->input->post('mother_name'));
            $data['mother_contact_no'] = html_escape($this->input->post('mother_contact_no'));
            $data['mother_email'] = html_escape($this->input->post('mother_email'));
    ...
    public function my_courses($today_date="")
    {
        $user_id = $this->session->userdata('user_id');
         //return $this->db->get_where('payment', array('user_id' => $user_id));
        $this->db->select('*');
        $this->db->from('payment');
        $this->db->where('schedule_expiry_date >=', $today_date , true);
        $this->db->where('user_id', $user_id , true);
        return $this->db->get();
    }
    public function my_batches($batch_id = "") {
           $user_id = $this->session->userdata('user_id');
        return $this->db->get_where('enrol', array('user_id' => $user_id,'batch_id' => $batch_id));
    }
    ...
    public function get_number_of_active_batch_of_instructor($instructor_id)
    {
        $checker = array(
            'user_id' => $instructor_id,
            'status'  => 'active'
