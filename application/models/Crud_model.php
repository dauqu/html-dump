...
class Crud_model extends CI_Model
{
    ...
    public function purchase_history_by_batch_id($batch_id)
    {
    ...
    public function get_payment_by_userid($user_id = "",$today_date = "")
    {
        $this->db->where('schedule_expiry_date >', $today_date);
        $this->db->where('user_id', $user_id);
        return $this->db->get('payment')->result_array();
    }

   public function get_payment_by_userid_group_by_batchid($user_id = "",$today_date = "")
    {
     //   $this->db->where('schedule_expiry_date >', $today_date);
        $this->db->where('user_id', $user_id);
        $this->db->group_by('batch_id'); 
        $this->db->order_by('schedule_expiry_date ', 'asc');  # or desc
        return $this->db->get('payment')->result_array();
    }

    public function get_payment_by_batch_id($user_id = "",$payment_id = "")
    {
    ...
    public function update_razorpay_settings()
    {
        ...
        $razorpay['razorpay_active'] = $this->input->post('razorpay_active');
        $razorpay['razorpay_mode'] = $this->input->post('razorpay_mode');
        $razorpay['razorpay_public_key'] = $this->input->post('razorpay_public_key');
        $razorpay['razorpay_secret_key'] = $this->input->post('razorpay_secret_key');
        $razorpay['razorpay_public_live_key'] = $this->input->post('razorpay_public_live_key');
        $razorpay['razorpay_secret_live_key'] = $this->input->post('razorpay_secret_live_key');
    ...
    public function get_batch_schedules($id = "")
    {
        $this->db->order_by("start_date", "desc");
        return $this->db->get_where('batch_schedules', array('batch_id' => $id));

    }

    public function get_student_batch_schedules($id = "",$schedule_start_date,$schedule_expiry_date)
    {
        $this->db->order_by("start_date", "desc");
        $this->db->where('start_date BETWEEN '.$schedule_start_date.' and '.$schedule_expiry_date);
        $this->db->where('batch_id' , $id);
        return $this->db->get('batch_schedules');
    }


    public function get_student_batch_attendance($schedule_id = "",$user_id="")
    {
    ...
    public function add_course($param1 = "")
    {
     ...
     //   $data['sessions'] = $this->input->post('sessions');
       // $data['session_details'] = $this->input->post('session_details');
        ...
        //$data['number_of_hours'] = $this->input->post('number_of_hours');
        $data['price'] = $this->input->post('course_price');
...
//print $_FILES[$course_media]['name'];die;
            if ($_FILES[$course_media]['name'] != "") {
    ...
    public function get_course_by_userid($course_id = "")
    {
        return $this->db->get_where('course', array('id' => $course_id));
    }
    ...
    public function delete_course($course_id)
    {
        ...
        // DELETE ALL THE BATCHES OF THIS COURSE FROM BATCHES TABLE
        $course_batches = array('course_id' => $course_id);
        $this->db->delete('batches', $course_batches);

        // DELETE ALL THE BATCH_SCHEDULES OF THIS COURSE FROM BATCH_SCHEDULES TABLE
        $batch_schedules = array('course_id' => $course_id);
        $this->db->delete('batch_schedules', $batch_schedules);

        // DELETE ALL THE ENROL OF THIS COURSE FROM ENROL TABLE
        $enrol = array('course_id' => $course_id);
        $this->db->delete('enrol', $enrol);

        $this->session->set_flashdata('flash_message', get_phrase('Course has been deleted successfully. '));
    ...
    public function get_section($type_by, $id)
    {
        ...
        }elseif ($type_by == 'batches')
	 {
            $sql="select b.* from batches b,batch_schedules bs where b.id = bs.batch_id and b.course_id =".$id." and bs.start_time > UNIX_TIMESTAMP() group by b.id";
            $query = $this->db->query($sql);
            return $query->result_array();
            //return  $this->db->get_where('batches', array('course_id' => $id));
        }
    }
    ...
    public function add_lesson()
    {
        ...
        public function edit_lesson($lesson_id)
        {
            ...
            public function enrol_student($user_id)
            {
                ...
                foreach ($purchased_courses as $k => $purchased_course) {

                    $data['user_id'] = $user_id;
                    $data['course_id'] = $k;
                    $data['batch_id'] = json_encode($purchased_course);
                    $data['date_added'] = strtotime(date('D, d-M-Y'));
            ...
            public function course_purchase($user_id, $method, $amount_paid)
            {
                ...
                foreach ($purchased_courses as $k => $purchased_course)
                {
                    ...
                    $data['course_id'] = $k;
                    $data['batch_id'] = json_encode($purchased_course);
                    $course_details = $this->get_course_by_id($k)->row_array();
                    ...
                    if (get_user_role('role_id', $course_details['user_id']) == 1) {
                        ...
                        if (get_settings('allow_instructor') == 1) {
                            ...
                            $data['instructor_revenue'] = ceil(($amount_paid * $instructor_revenue_percentage) / 100);
                            $data['admin_revenue'] = $amount_paid - $data['instructor_revenue'];
                        } else {
                            $data['instructor_revenue'] = 0;
                            $data['admin_revenue'] = $amount_paid;
                        }
            ...
            function curl_request($code = '')
            {
                ...
                function get_razorpay_supported_currencies()
                {
                    $this->db->where('razorpay_supported', 1);
                    return $this->db->get('currency')->result_array();
                }
